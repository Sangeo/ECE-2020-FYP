	...
	while (true) {
		...
		if (capture.read(newFrame)) {
			if (!newFrame.empty()) {
				...
				//keep a golden frame
				newFrame.copyTo(goldFrame);
				ROI = detectAndDisplay(newFrame);
				...
				if (initialising) {
					...
					for (i = k = 0; i < points[1].size(); i++) {
						...
						if (ROI.contains(points[1][i])) {
							...
							circle(newFrame, points[1][i], 3, Scalar(0, 255, 0), -1, 8);
						}
						else {
				...
				cv::rectangle(newFrame, ROI, Scalar(0, 255, 0));
				cv::imshow("Capture", newFrame);
