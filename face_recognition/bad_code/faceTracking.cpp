...
int main(int argc, const char** argv) {
	...
	int capLength = cv::CAP_PROP_FPS * 8;
	bool firstTime = true;
	...
	for (;;) {
		...
		if (frameQueue.size() > capLength / 2 && firstTime) {
			frameQueue.clear();
			firstTime = false;
		}
		else if (frameQueue.size() > capLength) {
			cout << "frameQueue size is currently: " << frameQueue.size() << endl;
			for (size_t i = 0; i < frameQueue.size() ; i++) {
				detectAndDisplay(frameQueue[i]);
				if (cv::waitKey(1) > 0) {
					break; // if escape is pressed at any time
				}
			}
			frameQueue.clear();
			cout << "Program has finished with processing all frames" << endl;
			break;
		}
		if (cv::waitKey(1) > 0) {
			break; // if escape is pressed at any time
...
Mat detectAndDisplay(Mat frame) {
	...
	if (cv::waitKey(1) > 0) {
		return trueROI; // if escape is pressed at any time
	}
	return trueROI;
}
...
Mat skinDetection(Mat frameC, Rect originalFaceRect) {
	...
	if (cv::waitKey(1) > 0) {
		return skin; // if escape is pressed at any time
	}
	return skin;
}
