...
#pragma omp for ordered schedule(static)
				for (int j = 0; j < numFrames; j++) {
					...
					std::tie(skinFrame, rBCG_ROI) = detectAndDisplay(frameQ[j].clone(), choice);

					if (!rBCG_ROI.empty()) {
						rBCG_ROI_Queue.push_back(rBCG_ROI);
					}
					else {
						std::cout << "rBCG_ROI is empty, replacing with previous sample" << std::endl;
					...
					if (!skinFrame.empty()) {
						...
						std::cout << "skinframe is empty, replacing with previous sample" << std::endl;
